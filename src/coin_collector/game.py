...
def rect_from_xywh(x: int, y: int, w: int, h: int) -> pygame.Rect:
...
def run_game(level_path: str, fps: int = 60, debug: bool = False) -> None:
    ...
    while running:
        ...
        # Normierung für diagonale Bewegungen (kein Diagonal-Speed-Vorteil)
        if vx and vy:
            vx *= 0.7071
            vy *= 0.7071
        ...
        for w in walls:
            ...
            if debug:
                pygame.draw.rect(screen, (200, 60, 60), w, width=1)

        # Coins
        for c in coins:
            pygame.draw.ellipse(screen, (255, 215, 0), c)
            if debug:
                pygame.draw.rect(screen, (60, 200, 60), c, width=1)

        # Spieler
        pygame.draw.rect(screen, (100, 200, 255), player, border_radius=6)
        if debug:
            pygame.draw.rect(screen, (255, 255, 255), player, width=1)

        # HUD
        total = score + len(coins)
        text = f"Score: {score} / {total} | Coins übrig: {len(coins)}"
        if debug:
